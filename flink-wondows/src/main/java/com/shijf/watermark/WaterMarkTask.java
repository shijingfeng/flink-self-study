...
public class WaterMarkTask {
    public static void main(String[] args) {
        ...
        //连接socket获取输入的数据
        DataStream<String> text = env.socketTextStream("cluster01", 9999, "\\W+");

        //解析输入的数据,每行数据按逗号分隔
        DataStream<Tuple2<String, Long>> inputMap = text.map(new MapFunction<String, Tuple2<String, Long>>() {
            @Override
            public Tuple2<String, Long> map(String value) throws Exception {
                return new Tuple2<>(value, new Date().getTime());
            }
        });
...
//                        sdf.format(currentMaxTimestamp) + "],watermark:[" + getCurrentWatermark().getTimestamp() + "|" + sdf.format(getCurrentWatermark().getTimestamp()) + "]");
        ...
        DataStream<String> window = waterMarkStream.keyBy(0)//分组
                .timeWindow(Time.seconds(3))//按照消息的EventTime分配窗口，和调用TimeWindow效果一样
